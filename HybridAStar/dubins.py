...
def plan_dubins_path(s_x, s_y, s_yaw, g_x, g_y, g_yaw, curvature,
    ...
    le_xy = np.stack([g_x - s_x, g_y - s_y]).T @ l_rot #np.stack([g_x - s_x, g_y - s_y]).T calculating the reative pose of gaol from the starting postion
                                                       # @ l_rot doing the relative trasformation to adopt the starting angle as origin
    local_goal_x = le_xy[0] #relative goal x and y
    local_goal_y = le_xy[1]
    local_goal_yaw = g_yaw - s_yaw # relative angle of the goal
...
def _calc_trig_funcs(alpha, beta):
    # calculating the sin and cosine of alpha(angle need to be moved from the start)  and beta (angle from the straignt path to the goal orientation)
    sin_a = sin(alpha)
    sin_b = sin(beta)
    cos_a = cos(alpha)
    cos_b = cos(beta)
...
def _LSL(alpha, beta, d):
    ...
    d1 = _mod2pi(-alpha + tmp) # distance from start to straight line starting (in the curve)
    d2 = sqrt(p_squared) # distance between straight line start to end
    d3 = _mod2pi(beta - tmp) # distance between the straight line end and goal position
...
def _dubins_path_planning_from_origin(end_x, end_y, end_yaw, curvature,
    ...
    dx = end_x
    dy = end_y
    d = hypot(dx, dy) * curvature # hypot calculates the eucledian distance while it is scaled by the curvature

    theta = _mod2pi(atan2(dy, dx)) # computing the angle connecting the starting and ending point
    alpha = _mod2pi(-theta) # intial orientation differnece
    beta = _mod2pi(end_yaw - theta) # final orientation difference

    best_cost = float("inf")
    b_d1, b_d2, b_d3, b_mode = None, None, None, None

    for planner in planning_funcs:
        """Loading the data into all path types and return the path having lowest cost"""
