...
from reeds_shepp import pi_2_pi,calc_paths
from dubins import _calc_trig_funcs
...
class Config:
    ...
    def __init__(self,grid, ox, oy, xy_resolution, yaw_resolution):
        self.min_x = 0
        self.min_y = 0
        self.max_x = len(grid)
...
def make_plan(grid,start, goal, is_generator,robot_footprint):
    """
    start: start node
    goal: goal node
    ...
    xy_resolution, yaw_resolution=XY_GRID_RESOLUTION,YAW_GRID_RESOLUTION
    ox,oy=[],[]
    for i in range(len(grid)):
        for j in range(len(grid[0])):
            if(grid[i][j]==1):
                ox.append(i)
                oy.append(j)
    start[2], goal[2] = pi_2_pi(np.deg2rad(start[2])), pi_2_pi(np.deg2rad(goal[2]))
    tox, toy = ox[:], oy[:]

    obstacle_kd_tree = cKDTree(np.vstack((tox, toy)).T)

    config = Config(grid,tox, toy, xy_resolution, yaw_resolution)
    ...
    h_dp = calc_distance_heuristic(
        grid, goal_node.x_list[-1], goal_node.y_list[-1],
...
def get_final_path(closed, goal_node):
    ...
    for i in range(len(reversed_x)):
        path.append((reversed_y[i],reversed_x[i],reversed_yaw[i]))
    return path
